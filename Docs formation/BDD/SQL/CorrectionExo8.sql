...
from @Commandes cd
inner join Customers  C on cd.IdClient = C.CustomerID
...
declare @nbCli int
select @nbCli = count(*) FROM Customers WHERE PostalCode IS NULL
...
--   * de 10 à 99 : + 10%
--   * 100 ou plus : + 3%
-- Vérifier les prix des produits notés à l'étape A
select * from Products where ProductID in (72, 73, 74)
begin tran
...
from @CASalaries cas
inner join EmployeeTerritories et on et.EmployeeID = cas.Id
